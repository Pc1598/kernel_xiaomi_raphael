		...
		struct dsi_parser_utils *utils)
{
	...
	if (count <= 0 || count % BRIGHTNESS_ALPHA_PAIR_LEN) {
		pr_debug("[%s] invalid number of elements %d\n",
			panel->name, count);
		rc = -EINVAL;
		goto count_fail;
	}
	...
	if (rc) {
		pr_debug("[%s] failed to read array, rc=%d\n", panel->name, rc);
		goto read_fail;
	}
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug("[%s] failed to parse dc dim lut\n", panel->name);
...
static int dsi_panel_sysfs_init(struct dsi_panel *panel)
{
	...
	if (rc)
		pr_debug("failed to create panel sysfs attributes\n");
